"""M3 Max calibration runner — runs 2A (TinyLlama) and 3C (Foxworthy F) only.

These two systems require >16GB RAM due to model copies during battery
instruments (ablation, perturbation, transfer control). The M3 Max with
36GB unified memory handles this comfortably.

Usage:
    .venv/bin/python scripts/run_m3_calibration.py

Outputs:
    results/calibration/*.json — individual BatteryResult files
    results/calibration/summary_m3.json — summary with all results
    results/calibration/m3_complete.flag — signals completion to Razer
...
def main():
    ...
    print(f"=== M8 Calibration — M3/M5 Max (2A + 3C only) ===")
        ...
        for seed in SEEDS:
            ...
            try:
                ...
                all_results.append({
                    ...
                    "error": str(e), "run_machine": "m3_max",
                })
            print()
    ...
    summary_path = RESULTS_DIR / f"summary_m3_{scale_name}.json"
    summary_path.write_text(json.dumps(summary, indent=2, default=str))

    # Completion flag
    flag_path = RESULTS_DIR / "m3_complete.flag"
