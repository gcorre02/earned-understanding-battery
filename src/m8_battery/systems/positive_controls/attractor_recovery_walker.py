...
class AttractorRecoveryWalker(TestSystem):
    ...
    def get_representation_state(self):
        """Edge weight + node consolidation vector for CKA."""
        edge_vals = np.array(list(self._weights.values()))
        node_vals = np.array([self._node_consolidation.get(n, 0.0) for n in sorted(self._graph.nodes())])
        combined = np.concatenate([edge_vals, node_vals])
